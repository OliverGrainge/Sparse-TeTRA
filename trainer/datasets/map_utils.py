...
def _download_map_image(
    min_lat=45.0, min_lon=7.6, max_lat=45.1, max_lon=7.7, size=2000
):
    ...
    return (
        np.array(image)[max_lat_px:min_lat_px, min_lon_px:max_lon_px],
        static_map,
        zoom,
    )
...
def get_edges(coordinates, enlarge=0):
    ...
    min_lat, min_lon, max_lat, max_lon = (
        *np.concatenate(coordinates).min(0),
        *np.concatenate(coordinates).max(0),
    )
...
def create_map(
    ...
    plot_img = cv2.resize(
        plot_img[:, :, :3], map_img.shape[:2][::-1], interpolation=cv2.INTER_LANCZOS4
    )
    map_img[(map_img.sum(2) < 444)] = 188  # brighten dark pixels
    map_img = (((map_img / 255) ** map_intensity) * 255).astype(np.uint8)  # fade map
    mask = (plot_img.sum(2) == 255 * 3)[
        :, :, None
    ]  # mask of plot, to find white pixels
