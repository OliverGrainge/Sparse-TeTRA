...
class EigenPlacesDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        for class_num in range(visualize_classes):
            ...
            focal_point_lat_lon = np.array(
                utm.to_latlon(focal_point[0], focal_point[1], 10, "S")
            )
            lats_lons = np.array([p.split("@")[5:7] for p in paths]).astype(float)
            lats_lons += (
                np.random.randn(*lats_lons.shape) / 500000
            )  # Add a little noise to avoid overlapping
            ...
            try:
                img_map = create_map(
                    [
                        lats_lons,
                        lats_lons.mean(0).reshape(1, 2),
                        focal_point_lat_lon.reshape(1, 2),
                        cell_corners,
                    ],
                    colors=["r", "b", "g", "orange"],
                    legend_names=[
                ...
                logging.warn(
                    "There was some problem while downloading the map of the class for visualization. "
    ...
    def initialize(dataset_folder, M, N, min_images_per_class, filename):
        ...
        class_id__group_id = [
            EigenPlacesDataset.get__class_id__group_id(*m, M, N)
        ...
        images_per_class = {
            k: v for k, v in images_per_class.items() if len(v) >= min_images_per_class
        }
    ...
    def get__class_id__group_id(utm_east, utm_north, M, N):
        ...
        group_id = (rounded_utm_east % (M * N) // M, rounded_utm_north % (M * N) // M)
